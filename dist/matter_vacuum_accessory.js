...
class RoborockMatterVacuumAccessory {
    ...
    getMatterCleanModeLiveCommandOptions() {
        return {
    ...
    reportUnmappedErrorCode(errorCode) {
        ...
        this.platform.log.info(`${this.getVacuumName()} reports error_code ${errorCode} while its state is ` +
            `${robotState !== null && robotState !== void 0 ? robotState : "unknown"} (Matter operationalState ${this.getOperationalState()}), ` +
    ...
    async changeCleanMode(newMode) {
        ...
        const previousSelection = {
            ...
            selectedCleanModeLiveConfirmationExpiresAt: this.selectedCleanModeLiveConfirmationExpiresAt,
        ...
        this.appliedCleanTypePin = null;
        const operationalState = this.getOperationalState();
        const canApplyLive = operationalState === RVC_OPERATIONAL_STATE.RUNNING ||
            operationalState === RVC_OPERATIONAL_STATE.PAUSED;
        try {
            if (canApplyLive) {
        ...
        this.setAndScheduleOptimisticState({ rvcCleanMode: { currentMode: newMode } }, "clean mode change");
    }
    ...
    doesLiveCleanModeMatch(cleanMode) {
    ...
    getCurrentCleanMode() {
        ...
        if (this.selectedCleanModeLiveConfirmationExpiresAt > 0) {
            if (!inCleaningRun) {
                ...
                this.selectedCleanModeLiveConfirmationExpiresAt = 0;
                this.appliedCleanTypePin = null;
                this.platform.log.warn(`Roborock acknowledged the live ${this.getCleanModeLabel(this.selectedCleanMode)} change for ${this.getVacuumName()}, but its status did not converge within ${MATTER_CLEAN_MODE_LIVE_CONFIRMATION_TIMEOUT_MS / 1000} seconds; Apple Home will now follow the robot's live report.`);
            }
        }
     ...
     */
    async applyCleanModeDuringRun(cleanMode) {
        var _a;
        ...
        if (!settings) {
            throw new Error(`No Roborock settings exist for ${this.getCleanModeLabel(cleanMode)}.`);
        }
        this.platform.log.info(`Applying ${this.getCleanModeLabel(cleanMode)} mode to ${this.getVacuumName()} during cleaning.`);
        const result = await this.withCleanModePrepTimeout(applySettings.call(this.api, this.getDuid(), settings, this.getMatterCleanModeLiveCommandOptions()));
        if ((_a = result === null || result === void 0 ? void 0 : result.unconfirmedSettings) === null || _a === void 0 ? void 0 : _a.length) {
            throw new Error(`Roborock did not confirm the ${result.unconfirmedSettings.join(" and ")} during the live clean-mode change.`);
        }
        this.selectedCleanModeNeedsApply = false;
        this.selectedCleanModeLiveConfirmationExpiresAt =
    ...
    clearOptimisticCluster(cluster) {
        const optimistic = this.getActiveOptimisticState();
