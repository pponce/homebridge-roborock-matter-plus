...
class RoborockPlatform {
     ...
     */
    removeHapScheduleSwitchAccessories() {
        ...
        const cached = this.accessories.filter((accessory) => (0, hap_schedule_accessory_1.isHapScheduleAccessory)(accessory));
        if (cached.length === 0) {
            return;
        }
        ...
        this.log.info(`Removing ${cached.length} HAP schedule accessor${cached.length === 1 ? "y" : "ies"}: schedule switches are switched off in the plugin settings.`);
        this.api.unregisterPlatformAccessories(settings_1.HAP_PLUGIN_IDENTIFIER, settings_1.PLATFORM_NAME, cached);
    }
    /**
     ...
     * alone.
     */
    ensureScheduleAccessoryRegistered(vacuumName, schedule) {
        ...
        this.accessories.push(accessory);
        this.log.info(`Adding HAP schedule accessory '${vacuumName} schedules' with ${schedule.scheduleCount} switch${schedule.scheduleCount === 1 ? "" : "es"}.`);
        this.api.registerPlatformAccessories(settings_1.HAP_PLUGIN_IDENTIFIER, settings_1.PLATFORM_NAME, [
            accessory,
        ]);
    }
            ...
            if (schedule) {
                ...
                void schedule
                    ...
                    if (result.success && result.hasSchedules) {
                        ...
                        this.ensureScheduleAccessoryRegistered(target.vacuumName, schedule);
                        return;
                    }
                    if (!result.success) {
