...
const mockClient = {
  ...
  publish: jest.fn(), end: jest.fn(), reconnect: jest.fn(), removeAllListeners: jest.fn(),
};
...
const { UnansweredMethodBreaker } = require("../roborockLib/lib/unansweredMethodBreaker");

let adapter, connector, queue, states, requestId;
const topic = (duid = "robot-a") => `rr/m/o/private-user/private-client/${duid}`;
const snapshot = () => JSON.parse(states.get("MqttSessionDiagnostics")?.val || "{}");
...
const reply = (id, result = ["ok"]) => ({
  protocol: 102, payload: JSON.stringify({ dps: { 102: JSON.stringify({ id, result }) } }),
});
async function startRequest(duid = "robot-a", method = "get_status", options = {}) {
  const result = queue.sendRequest(duid, method, [], false, false, options).catch((error) => error);
...
beforeEach(async () => {
  ...
  jest.clearAllMocks(); mockHandlers.clear(); mockSubscriptions.length = 0;
  states = new Map(); requestId = 0;
  adapter = {
    config: {},
    log: { debug: jest.fn(), info: jest.fn(), warn: jest.fn(), error: jest.fn() },
    localKeys: new Map([["robot-a", "private-key-a"], ["robot-b", "private-key-b"]]),
    devices: [{ duid: "robot-a" }, { duid: "robot-b" }],
    pendingRequests: new Map(), pendingB01MapRequests: new Map(),
    ...
    noteRequestAnswered: jest.fn(), noteRequestUnanswered: jest.fn(),
    localConnector: { isConnected: jest.fn(() => false), sendMessage: jest.fn(), clearChunkBuffer: jest.fn() },
    message: {
  ...
  await connector.initUser({ rriot: { u: "private-user", k: "private-key", s: "private-secret", r: { m: "mqtts://broker.example" } } });
  await connector.initMQTT_Subscribe();
  await connector.initMQTT_Message();
  mockHandlers.get("connect")({ sessionPresent: false });
  acknowledge();
});
afterEach(() => { connector.disconnect(); jest.clearAllTimers(); jest.useRealTimers(); });

test("export records connection generation and actual SUBACK, not merely a socket", () => {
  expect(snapshot()).toMatchObject({ generation: 1, connected: true, subscriptionAcknowledged: true });
...
test.each([new Error("denied"), null])("failed or rejected subscription is recorded (%s)", (error) => {
  mockHandlers.get("connect")({ sessionPresent: false });
...
test("raw, attributed, decoded and matched reply ages distinguish delivery stages", async () => {
  ...
  expect(snapshot()).toMatchObject({ lastRawInboundAgeMs: 0, lastAttributedInboundAgeMs: null, lastDecodedInboundAgeMs: null, lastCorrelatedReplyAgeMs: null });
  await jest.advanceTimersByTimeAsync(30_000);
  receive(null);
  expect(snapshot()).toMatchObject({ lastAttributedInboundAgeMs: 0, lastDecodedInboundAgeMs: null });
  await jest.advanceTimersByTimeAsync(30_000);
  receive(reply(77));
  expect(snapshot()).toMatchObject({ lastDecodedInboundAgeMs: 0, lastCorrelatedReplyAgeMs: null });
...
test("two active silent robots are observed in the production timeout and persisted export", async () => {
  ...
  expect(first).toMatchObject({ unansweredRequest: true, transportWasUp: true });
  expect(snapshot()).toMatchObject({ silentReadRobotCount: 1, correlatedSilenceObserved: false });
  const second = await timeout("robot-b");
  expect(snapshot()).toMatchObject({ silentReadRobotCount: 2, correlatedSilenceObserved: true });
  expect(second.message).toContain('"correlatedSilenceObserved":true');
  expect(adapter.noteRequestUnanswered).toHaveBeenCalledWith("robot-b", "get_status", second);
...
test("idle cloud time and repeated silence from one robot do not imply account silence", async () => {
  ...
  await timeout("robot-a"); await timeout("robot-a");
  expect(snapshot()).toMatchObject({ silentReadRobotCount: 1, correlatedSilenceObserved: false });
  expect(mockClient.reconnect).not.toHaveBeenCalled();
});
...
test("any raw MQTT traffic clears correlation even when attribution fails", async () => {
  ...
  expect(snapshot()).toMatchObject({ silentReadRobotCount: 0, correlatedSilenceObserved: false });
});
...
test("an in-flight request from the previous generation cannot seed correlation", async () => {
  ...
  await timeout("robot-b");
  expect(snapshot()).toMatchObject({ generation: 2, silentReadRobotCount: 1, correlatedSilenceObserved: false });
});
...
test("a down transport and ambiguous writes do not contribute silent-read evidence", async () => {
  ...
  mockHandlers.get("connect")({ sessionPresent: false }); acknowledge();
  await timeout("robot-a", "app_start"); await timeout("robot-b", "app_start");
  expect(snapshot()).toMatchObject({ silentReadRobotCount: 0, correlatedSilenceObserved: false });
});

test("local replies continue and are recorded separately during cloud silence", async () => {
  await timeout("robot-a"); await timeout("robot-b");
  ...
  adapter.localConnector.sendMessage.mockImplementation(() => {
    ...
    clearTimeout(pending.timeout); adapter.pendingRequests.delete(requestId);
  ...
  await timeout("robot-b");
  expect(snapshot()).toMatchObject({ lastLocalReplyAgeMs: 10_000, lastCorrelatedReplyAgeMs: null, correlatedSilenceObserved: true });
});

test("breaker behavior is unchanged even when correlation is observed", async () => {
  const breaker = new UnansweredMethodBreaker();
  for (const duid of ["robot-a", "robot-b"]) breaker.govern(duid, "get_consumable");
  adapter.noteRequestUnanswered.mockImplementation((duid, method, error) => breaker.recordFailure(duid, method, error));
  for (let i = 0; i < 6; i++) {
    await timeout("robot-a", "get_consumable"); await timeout("robot-b", "get_consumable");
  }
...
test.each(["b01", "b01-map", "secure-ack", "refusal"])("matched %s replies count without altering completion semantics", async (kind) => {
  await jest.advanceTimersByTimeAsync(30_000);
  const resolve = jest.fn(), reject = jest.fn();
  if (kind === "b01") {
    adapter.pendingRequests.set("7", { resolve, reject });
    receive({ protocol: 102, payload: JSON.stringify({ dps: { 10001: { msgId: "7", code: 0, data: { status: 1 } } } }) });
    ...
    adapter.pendingRequests.set(7, { resolve, reject, secure: kind === "secure-ack" });
    if (kind === "secure-ack") { receive(reply(7)); expect(resolve).not.toHaveBeenCalled(); }
    else {
      receive({ protocol: 102, payload: JSON.stringify({ dps: { 102: JSON.stringify({ id: 7, error: { code: -1, message: "refused" } }) } }) });
...
test("a diagnostics write failure cannot stop a real reply resolving", async () => {
  adapter.setStateAsync.mockImplementation(async () => { throw new Error("disk unavailable"); });
  const request = await startRequest();
...
test("the persisted observation reaches the actual UI route and copied report without identities", async () => {
  ...
  try {
    for (const [key, value] of states) fs.writeFileSync(path.join(storage, `roborock.${key}`), JSON.stringify(value));
    fs.writeFileSync(path.join(storage, "roborock.HomeData"), JSON.stringify({ val: JSON.stringify({ devices: [], products: [] }) }));
    const handlers = new Map();
    class UiHost {
      constructor() { this.homebridgeStoragePath = storage; }
      onRequest(route, handler) { handlers.set(route, handler); }
      ready() {}
    }
    const filename = path.join(__dirname, "../src/ui/index.ts");
    const js = ts.transpileModule(fs.readFileSync(filename, "utf8"), { compilerOptions: { module: ts.ModuleKind.CommonJS, esModuleInterop: true } }).outputText;
    const mod = { exports: {} };
    const { createRequire } = require("module");
    const context = { module: mod, exports: mod.exports, require: createRequire(filename), __dirname: path.dirname(filename), process, console, Buffer };
    ...
    expect(result.mqttSession).toMatchObject({ generation: 1, correlatedSilenceObserved: true });
    const browser = fs.readFileSync(path.join(__dirname, "../homebridge-ui/public/index.js"), "utf8");
    const from = browser.indexOf("async function buildDiagnosticsReport");
    const to = browser.indexOf("function appendLocalTestReport");
    const ctx = {
      state: {}, maskIdentifier: (x) => x, maskLocalIp: (x) => x,
    ...
    expect(report).not.toMatch(/private-user|private-client|private-secret|private-key|robot-a|robot-b|rr\/m/);
