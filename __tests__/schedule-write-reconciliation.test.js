...
jest.mock("mqtt", () => ({ connect: jest.fn(() => {
  ...
  const client = {
    ...
    publish: jest.fn((topic, payload) => mockPublish(client, JSON.parse(payload.toString()))),
    end: jest.fn(), endAsync: jest.fn(async () => {}), reconnect: jest.fn(),
...
let adapter, connector, queue, coordinator, timers, writes, reads, events, nextId, replacements;
const request = (enabled = true, scheduleId = "timer-1") => ({ scheduleId, enabled });
const snapshot = () => [...timers].map(([id, enabled]) => [id, enabled ? "on" : "off"]);
function reply(client, id, result, error) {
  adapter.message._decodeMsg.mockReturnValue({
    protocol: 102,
    payload: JSON.stringify({ dps: { 102: JSON.stringify({ id, result, ...(error ? { error } : {}) }) } }),
  });
  client.handlers.get("message")?.("rr/m/o/user/client/robot-a", Buffer.from("reply"));
}
async function run(requests = [request()]) {
  let result;
  const operation = coordinator.executeScheduleWriteBatch(requests).then(value => { result = value; });
...
beforeEach(async () => {
  ...
  adapter = {
    ...
    localKeys: new Map([["robot-a", "key"]]), devices: [{ duid: "robot-a" }],
    pendingRequests: new Map(), pendingB01MapRequests: new Map(),
    isRemoteDevice: async () => false, getRobotVersion: async () => "1.0",
    onlineChecker: async () => true, getRequestId: () => ++nextId,
    setTimeout, clearTimeout, setStateAsync: async () => {},
    updateTransportDiagnostics: async () => {}, catchError: jest.fn(),
    localConnector: { isConnected: () => false, clearChunkBuffer: jest.fn() },
    message: {
      _decodeMsg: jest.fn(),
      buildPayload: async (duid, protocol, id, method, params) => ({ id, method, params }),
      buildRoborockMessage: async (duid, protocol, timestamp, payload) => Buffer.from(JSON.stringify(payload)),
    },
  };
  connector = new roborock_mqtt_connector(adapter);
  adapter.rr_mqtt_connector = connector;
  queue = new messageQueueHandler(adapter);
  adapter.getServerTimers = (duid, options) => queue.sendRequest(duid, "get_server_timer", [], false, false, options);
  adapter.getCloudScenes = jest.fn(async () => []);
  adapter.vacuums = { "robot-a": { command: (duid, method, params, options) => queue.sendRequest(duid, method, params, false, false, options) } };

  mockPublish = (client, message) => {
    events.push(message);
    if (message.method.startsWith("upd_")) {
      const action = writes.shift() || { apply: true, reply: true };
      const [id, state] = message.method === "upd_server_timer" ? message.params[0] : message.params;
      ...
      action.effect?.();
      if (!action.silent) reply(client, message.id, "value" in action ? action.value : snapshot());
    }
  };
  await connector.initUser({ rriot: { u: "user", s: "secret", k: "key", r: { m: "mqtts://broker.example" } } });
  ...
  Object.assign(coordinator, {
    platform: { roborockAPI: adapter, log: adapter.log }, duid: "robot-a",
    scheduleAccessories: new Map(), routineSwitches: new Map(),
    managerAccessory: {}, vacuumName: "Robot", disposed: false,
    cachedSchedules: [{ id: "timer-1", enabled: false, timer: ["timer-1", "off"] }],
    lastServerTimerSchedules: [{ id: "timer-1", enabled: false, timer: ["timer-1", "off"] }],
    lastCloudSceneSchedules: [], lastScheduleRefreshAt: 0,
    lastFailedRefreshAt: 0, consecutiveRefreshFailures: 0, nextRefreshAttemptAt: 0,
    scheduleBackoffRandom: () => 0.5, accountCoordinator: new ScheduleAccountCoordinator(),
    refreshGeneration: 0, writeBatcher: { cancelPending: jest.fn() },
    sync: jest.fn(), syncRoutines: jest.fn(),
    waitForScheduleVerification: async () => {}, waitForScheduleWriteSpacing: async () => {},
  });
});
afterEach(async () => {
...
test.each([true, false])("lost acknowledgement, assignment applied (%s): read confirms it without retry", async enabled => {
  timers.set("timer-1", !enabled);
  writes.push({ apply: true, reply: false });
  const failures = await run([request(enabled)]);
  expect(failures.size).toBe(0);
  expect(events.map(e => e.method)).toEqual(["upd_server_timer", "get_server_timer"]);
...
test("lost acknowledgement, assignment not applied: read precedes one fallback and final verification", async () => {
  ...
  expect(failures.size).toBe(0);
  expect(events.map(e => e.method)).toEqual(["upd_server_timer", "get_server_timer", "upd_timer", "get_server_timer"]);
});
...
test.each([[], "invalid", [[null]]])("missing or malformed read does not authorize another write: %j", async value => {
  writes.push({ apply: false, reply: true });
  reads.push({ value });
  expect((await run()).has("timer-1")).toBe(true);
  expect(writeEvents()).toHaveLength(1);
...
test("failure of the unrelated cloud-scene read does not invalidate a fresh timer confirmation", async () => {
  adapter.getCloudScenes.mockRejectedValue(new Error("scene service unavailable"));
  writes.push({ apply: true, reply: false });
  expect((await run()).size).toBe(0);
  expect(writeEvents()).toHaveLength(1);
});
...
test("final failed read cannot turn a stale desired state into success", async () => {
  writes.push({ apply: false, reply: true }, { apply: true, reply: true, effect: () => {
    coordinator.lastServerTimerSchedules[0].enabled = true;
  } });
  reads.push({}, { silent: true });
  expect((await run()).has("timer-1")).toBe(true);
...
test("an explicit robot refusal is not retried", async () => {
  writes.push({ apply: false, error: { code: -10007, message: "Not FCC robot" } });
  expect((await run()).has("timer-1")).toBe(true);
  expect(writeEvents()).toHaveLength(1);
});
...
test("shutdown after an ambiguous write prevents both verification and retry", async () => {
  writes.push({ apply: false, reply: false, effect: () => { coordinator.disposed = true; } });
  expect((await run()).has("timer-1")).toBe(true);
...
test("shutdown during verification prevents fallback", async () => {
  writes.push({ apply: false, reply: true });
  reads.push({ effect: () => { coordinator.disposed = true; } });
  expect((await run()).has("timer-1")).toBe(true);
  expect(writeEvents()).toHaveLength(1);
});

test("verification does not adopt a read started before the write completed", async () => {
  coordinator.refreshInProgress = Promise.resolve({ success: true, hasSchedules: true });
