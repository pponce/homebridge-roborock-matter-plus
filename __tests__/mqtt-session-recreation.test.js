...
test("TCP connect alone cannot allow a write before the subscription is acknowledged", async () => {
  ...
  const { result } = await startRequest("robot-a", "app_start", { preferCloud: true });
  await jest.advanceTimersByTimeAsync(10_000);
  expect(await result).toMatchObject({ requestNotSent: true, transientKind: "cloud unavailable" });
...
test("pending cloud writes reject with an unknown outcome, without replay; local requests survive", async () => {
  ...
  await second.result;
  expect(mockClients).toHaveLength(2);
  expect(await write.result).toMatchObject({ code: "MQTT_SESSION_REPLACED", transientKind: "mqtt session replaced", unansweredRequest: false });
