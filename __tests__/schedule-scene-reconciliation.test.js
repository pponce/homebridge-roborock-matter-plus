...
beforeEach(async () => {
  events = []; puts = 0; gets = 0;
  behavior = { apply: true, drop: true };
  scene = {
    id: 7, name: "Morning", enabled: true, type: "WORKFLOW",
    param: JSON.stringify({
      triggers: [{ id: 1, type: "TIMER", param: JSON.stringify({ cron: "0 9 * * *", enabled: false, repeated: true, timeZoneId: "UTC" }) }],
      action: { type: "S", items: [] },
    }),
  };
  server = http.createServer((req, res) => {
    ...
    if (req.method === "OPTIONS") { res.writeHead(200, { Allow: "GET, PUT, OPTIONS" }); res.end(); return; }
    if (req.method === "GET") {
      gets++;
      if (behavior.failReads || (behavior.failVerification && gets >= 2)) send(behavior.readStatus || 503, { success: false });
    ...
    req.on("end", () => {
      ...
      puts++;
      if (behavior.throttle) { send(429, { success: false }); return; }
      if (puts > 1 || behavior.apply) scene.param = body;
      if (puts === 1 && behavior.externalEdit) {
        const param = JSON.parse(scene.param);
        param.action = { type: "S", items: [{ name: "edited in app", type: "CMD", param: JSON.stringify({ method: "app_start", params: [] }) }] };
  ...
  api.api = axios.create({ baseURL: `http://127.0.0.1:${server.address().port}/`, timeout: 1000, proxy: false });
  api.log = { debug: jest.fn(), info: jest.fn(), warn: jest.fn(), error: jest.fn() };
  api.getServerTimers = async () => [];
  coordinator = Object.create(moduleUnderTest.default.prototype);
  Object.assign(coordinator, {
    platform: { roborockAPI: api, log: api.log }, duid: "robot-a", vacuumName: "Robot",
    scheduleAccessories: new Map(), routineSwitches: new Map(), disposed: false,
    cachedSchedules: [], lastServerTimerSchedules: [], lastCloudSceneSchedules: [],
    refreshGeneration: 0, lastScheduleRefreshAt: 0, consecutiveRefreshFailures: 0,
...
const run = () => coordinator.executeScheduleWriteBatch([{ scheduleId: "scene:7", enabled: true }]);
...
test("a partial scene enable finishes the top-level flag only after read-back", async () => {
  ...
  expect(puts).toBe(2);
  expect(events.filter(e => e === "PUT /user/scene/7/enable")).toHaveLength(1);
});
...
test("an acknowledged scene PUT is not confirmed using cached state after a failed read", async () => {
  ...
  coordinator.lastCloudSceneSchedules = [{ id: "scene:7", enabled: true, timer: [] }];
...
test("a scene-read throttle stops timer fallback even when the timer source answered", async () => {
  ...
  const failures = await coordinator.executeScheduleWriteBatch([{ scheduleId: "timer-1", enabled: true }]);
